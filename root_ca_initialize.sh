...
#
# Generate root Certificate Authrority (CA) OCSP responder certificate
if [ ! -f /root/ca/certs/ocsp.guardtone.com.crt.pem ]; then
	printf "\n>> Generating root CA OCSP responder certificate and self signing...\n\n"
	openssl req -config /root/ca/openssl_root.cnf -new -x509 -sha384 -extensions ocsp -key /root/ca/private/ocsp.guardtone.com.key.pem -out /root/ca/certs/ocsp.guardtone.com.crt.pem
fi
